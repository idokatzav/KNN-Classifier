...
UploadCommand::UploadCommand(DefaultIO *dio, ClassifierData *classifierData) : Command(dio, classifierData) {
    m_description = "uploadData an unclassified csv data file";
}

void UploadCommand::execute() {
    if (!m_classifierData->classifier().isInit()) {
        m_dio->write("Please uploadData your local train CSV file.");
        ...
        m_dio->write("Please uploadData your local test CSV file.");
